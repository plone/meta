...
`plone.meta` provides a default combination (the same as used by Plone itself), but it can be overridden in {file}`.meta.toml`:
...
This generates the necessary `tox` environments and the GitHub Actions that run on each pull request.
...
The `constraints_file` option in {file}`.meta.toml`'s `[tox]` table was renamed to `constraints_files`, and the type of its value was changed from a string to a dictionary.
...
Use the new `test_matrix` option from the `[tox]` table instead, as plone.meta can now run multiple Python versions from within `tox` itself.
...
The `custom_image` option in {file}`.meta.toml`'s `[gitlab]` table was renamed to `custom_images`, and the type of its value was changed from a string to a dictionary.
